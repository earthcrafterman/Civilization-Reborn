...
#####################################  
	...
	def checkTurn(self, iGameTurn):
		...
		if iGameTurn == getTurnForYear(330):
			if pMongolia.isHuman() and pXiongnu.isAlive():
				sta.completeCollapse(iXiongnu)
	...
	def initBirth(self, iCurrentTurn, iBirthYear, iCiv): # iBirthYear is really year now, so no conversion prior to function call - edead
		...
		if iSpawnType == iConditionalSpawn and iCiv in lConditionalCivs:
			...
			if iCiv == iByzantium:
				...
				if pWari.isAlive():
					if utils.getHumanID() != iWari:
						if data.getStabilityLevel(iWari) > iStabilityShaky:
							return
					else:
						if data.getStabilityLevel(iWari) > iStabilityUnstable:
							return
